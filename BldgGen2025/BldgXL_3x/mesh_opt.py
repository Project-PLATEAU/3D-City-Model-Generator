...
class meshOPTCrossAttentionFlashAttn2(OptFlashAttention2):
...
OPT_ATTENTION_CLASSES = {
    ...
    "flash_cross_attention_2": meshOPTCrossAttentionFlashAttn2
}   
...
class meshOPTDecoderLayer(OPTDecoderLayer):
    def __init__(self, config: OPTConfig):
        ...
        if config._attn_implementation == "flash_attention_2":
            self.cross_attn = meshOPTCrossAttentionFlashAttn2(config=config, is_decoder=True)
        else:
            self.cross_attn = meshOPTCrossAttention(config=config, is_decoder=True)
